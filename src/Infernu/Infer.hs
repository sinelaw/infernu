...
import           Infernu.Expr       (Exp(..), LitVal(..), EVarName, getAnnotations)
...
closeRow a (Fix (TApp (TCons TRecord k) [t])) = (\t' -> Fix (TApp (TCons TRecord k) [t'])) <$> closeRow a t
...
--     unify a thisT (closeRow resolvedThisT)
     ...
     let thisT' = TQual preds' thisTLabeled
         thisTLabeled = case resolvedThisT of
                            Fix (TApp (TCons TRecord k) [Fix (TRow Nothing rl)]) -> record (label e1) rl
                            _ -> resolvedThisT
...
inferType' env (EArray a exprs) =
  do tv <- Fix . TBody . TVar <$> freshFlex KStar
     ...
     let t = qualEmpty $ Fix $ TApp (TCons TArray (KArrow KStar KStar)) [tv]
     return (t, EArray (a,t) $ map snd te)
inferType' env (ETuple a exprs) =
  do te <- accumInfer env exprs
     let t = TQual (concatMap (qualPred . fst) te)
             $ Fix
             . TApp (TCons TTuple ((KArrow KStar (KArrow KStar KStar))))
             . reverse $ map (qualType . fst) te
...
inferType' env (EStringMap a exprs') =
  do let exprs = map snd exprs'
     ...
     let t = TQual
             { qualPred = allPreds,
               qualType = Fix $ TApp (TCons TStringMap (KArrow KStar KStar)) [elemType]
             }
